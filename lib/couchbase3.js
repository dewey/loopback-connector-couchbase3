...
Couchbase.prototype.exists = function (model, id, callback) {
  var self = this;
  debug("Couchbase.prototype.exists()");

  var bucket = self.cluster.openBucket(self.settings.bucket, function (err) {
    if (err) {
      callback(err, null);
      return;
    }
    ...
    bucket.get(id, function (err, result) {
      if (err) {
        debug("Couchbase.prototype.exists() : " + err.message);
...
Couchbase.prototype.find = function find(model, id, callback) {
  var self = this;
  debug("Couchbase.prototype.find()");
  ...
  var bucket = self.cluster.openBucket(self.settings.bucket, function (err) {
    ...
    bucket.get(id, function (err, result) {
      if (err) {
        debug("Couchbase.prototype.find() : " + err.message);
        callback(err, null);
        return;
      } else {
        callback(err, result);
